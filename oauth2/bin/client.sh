...
function create_client {
    ...
    curl -sSLk -X POST $HYDRA_ADMIN_URI/clients -d @$client_json | jq --indent 4 .
}

function get_client {
    local client_id=${1:?ERROR: mandatory client_id is not provided}

    curl -sSLk -X GET $HYDRA_ADMIN_URI/clients/$client_id | jq --indent 4 .
}

function delete_client {
    local client_id=${1:?ERROR: mandatory client_id is not provided}

    curl -sSLk -X DELETE $HYDRA_ADMIN_URI/clients/$client_id
}

function get_all_clients {
    curl -sSLk -X GET $HYDRA_ADMIN_URI/clients | jq --indent 4 .
}
